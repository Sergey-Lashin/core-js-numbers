...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distanceOfX = (x2 - x1) ** 2;
  const distanceOfY = (y2 - y1) ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const A = x1 * x2;
  const B = y1 * y2;
  const scalarProductOfVectors = A + B;
  const lengthOfVectorA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lengthOfSecondB = Math.sqrt(x2 ** 2 + y2 ** 2);
  const angleBetweenVectors =
    scalarProductOfVectors / (lengthOfVectorA * lengthOfSecondB);
...
function roundToPowerOfTen(num, pow) {
  const modifiedNum = Math.round(num / 10 ** pow);
  return modifiedNum * 10 ** pow;
}
...
function getSumOfDigits(num) {
  ...
  while (i < stringOfNum.length) {
    result += +stringOfNum[i];
    i += 1;
  }
  return result;
}
...
function isPowerOfTwo(num) {
  const logOfNum = Math.log2(num);
  const truncNum = Math.trunc(logOfNum);
  return logOfNum === truncNum;
}
